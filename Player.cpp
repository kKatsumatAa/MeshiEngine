...
void Player::Draw(ViewMat& view, ProjectionMat& projection, const UINT64* texHundle, const UINT64* numTexHundle)
{
	...
	if (!isDead)
	{
		if (status == TARGET)
		{
			...
			d.DrawCube3D(&worldMat, &view, &projection, { 1.0f,1.0f,1.0f,0.2f }, texHundle[0]);
			draw.DrawBoxSprite({ pos.x,pos.y,0 }, 0.25f, { 0.8f, 0.8f, 0.8f, 0.8f * coolColor }, texHundle[4], { 0.5f,0.5f });
