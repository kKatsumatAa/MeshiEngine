...
void CollisionManager::CheckCollisionPair2(Collider* colliderA, Collider* colliderB)
{
	if (colliderA->isLockOn && !colliderA->isDead && !colliderB->isDead)//playerがロックオンモードの時のみ
	{
		...
		if (!(colliderA->GetCollisionAttribute() == kCollisionAttributePlayer
			...
			|| colliderB->isLockOned >= 10 || colliderB->HP <= colliderB->isLockOned)//Aがplayerで、Bが敵の時のみ処理
		{
			return;//判定、衝突処理せず抜ける
		}
		...
		float rB;
		if (colliderB->isBoss)
			rB = colliderB->GetRadius() * (posB.GetLength()+100) / 150.0f;//仮
		else
			rB = colliderB->GetRadius() * posB.GetLength() / 50.0f;//仮
