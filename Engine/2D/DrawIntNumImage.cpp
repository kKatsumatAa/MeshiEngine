	...
	if (isStartDigit_)
	{
		...
		for (int32_t i = numCount_ - 1; i >= 0; i--)
		{
			if (numImages_[i].isTrue)
			{
				...
				numImages_[i].obj.DrawClippingBoxSprite({ pos.x,pos.y,0 }, numImages_[i].scale, { numImages_[i].num * numImages_[i].sizeUV.x,0 },
					{ numImages_[i].sizeUV.x,numImages_[i].sizeUV.y }, numImages_[i].color, texhandle_, true);
			}
		}
	}
}
