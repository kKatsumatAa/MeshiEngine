...
Directx::Directx(const WindowsApp& win)
{
#ifdef _DEBUG
	//デバッグレイヤーをオンに
...
void Directx::DrawInitialize(const WindowsApp& win)
{
...
void Directx::PipeLineState(const D3D12_FILL_MODE& fillMode, ID3D12PipelineState** pipelineState)
{
	// シェーダーの設定
...
void Directx::Blend(const D3D12_BLEND_OP& blendMode, const bool& Inversion, const bool& Translucent)
{
	//共通設定
...
void Directx::constBuffTransfer(const XMFLOAT4& plusRGBA)
{
	color2.x += plusRGBA.x;
	color2.y += plusRGBA.y;
	color2.z += plusRGBA.z;
...
void Directx::ResourceProperties(D3D12_RESOURCE_DESC& resDesc, const UINT& size)
{
...
void Directx::BuffProperties(D3D12_HEAP_PROPERTIES& heap, D3D12_RESOURCE_DESC& resource, ID3D12Resource** buff)
{
...
void Directx::SetNormDigitalMat(XMMATRIX& mat, const WindowsApp& win)
{
...
void Directx::Error(const bool& filed)
{
	if (filed)
	{
