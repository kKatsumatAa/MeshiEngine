...
void GameVelocityManager::Update()
{
	...
	Vec3 col = LerpVec3({ VEL_COLOR_MAX_.x,VEL_COLOR_MAX_.y,VEL_COLOR_MAX_.z },
		VEL_COLOR_MIN_, EaseInOut(GAME_VELOCITY_MAX_ - gameVelocity_));
	PostEffectManager::GetInstance().GetPostEffect2()->effectFlags_.color = { col.x,col.y,col.z,VEL_COLOR_MAX_.w };
	...
	if ((1.0f - gameVelocity_ / GAME_VELOCITY_MAX_) * VEL_RADIAL_BLUR_RATE_ >= 0)
	{
		...
		PostEffectManager::GetInstance().GetPostEffect2()->effectFlags_.radialPow = (GAME_VELOCITY_MAX_ - gameVelocity_ / GAME_VELOCITY_MAX_) * VEL_RADIAL_BLUR_RATE_;
	}
	else
	{
		PostEffectManager::GetInstance().GetPostEffect2()->effectFlags_.isRadialBlur = false;
